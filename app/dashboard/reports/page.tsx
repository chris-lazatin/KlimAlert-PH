  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-zinc-400 mt-1 max-w-2xl">
              Bawat residente ay isang sensor. Ang verified reports ay
              ipinapadala sa LGU at sa mga kalapit na barangay.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-lg border border-amber-500/30 bg-amber-500/6 px-3 py-1.5 text-xs text-amber-200">
              <Clock className="h-3.5 w-3.5" />
              {pending} pending review
            </span>
            <span className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-500/30 bg-emerald-500/6 px-3 py-1.5 text-xs text-emerald-200">
              <ShieldCheck className="h-3.5 w-3.5" />
              {verified} verified
            </span>
            <span className="inline-flex items-center gap-1.5 rounded-lg border border-sky-500/30 bg-sky-500/6 px-3 py-1.5 text-xs text-sky-200">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_460px] gap-5">
          ...
          <ul className="divide-y divide-zinc-900">
            ...
            {filtered.map((r) => {
              ...
              return (
                <li
                  key={r.id}
                  className="px-5 py-4 hover:bg-zinc-900/30 transition-colors"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-zinc-500 mt-1 inline-flex items-center gap-1.5">
                        ...
                        Brgy. {r.barangay}
                        {r.landmark && (
                          <span className="text-zinc-600">· {r.landmark}</span>
                        )}
                      </p>
        ...
        <aside
          id="report"
          className="scroll-mt-24 lg:sticky lg:top-5 lg:self-start"
        >
          <HazardReportForm />
        </aside>
      </div>
